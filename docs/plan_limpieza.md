...
Este plan es **solo planificación** (Paso 3): no ejecuta limpieza por sí mismo. La ejecución posterior vive en el pipeline (`src/proyecto1_ds/`): normalización y bitácora; catálogo territorial; 2 variantes/19 filas corregidas; 7 parejas/145 filas `revisar`; 11 pares independientes aprobados sin fusión; y 6 teléfonos normalizados por manifiesto. Permanecen 718 `duplicado_probable`, 260 pares ambiguos y 245 teléfonos.
...
| `TELEFONO` | 946 faltantes; el diagnóstico inicial conserva 201 hallazgos históricos agregados. Tras 6 decisiones exactas quedan 245 sospechosos vigentes, incluidos 50 numéricos con longitud distinta. | Aplicar solo `data/decisions/telefonos_aprobados.csv`; conservar los demás como texto y no inferir correspondencia desde el agregado histórico. | El manifiesto hace auditables las únicas transformaciones aprobadas. | Alto fuera del manifiesto; bajo para las 6 reglas exactas aprobadas. | `manual` | Diagnóstico histórico: `problemas_potenciales.csv`; decisiones: `data/decisions/telefonos_aprobados.csv`; control vigente: `validacion_final.csv`. |
