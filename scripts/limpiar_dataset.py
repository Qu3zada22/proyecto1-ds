...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--interim-csv", type=Path, default=ROOT / DEFAULT_INTERIM_CSV)
...
def _resolve_interim_csv(interim_csv: Path) -> Path:
    interim_root = Path(os.path.normpath(ROOT / "data/interim"))
    source = _absolute_lexical_path(interim_csv)
    try:
        relative_source = source.relative_to(interim_root)
    except ValueError:
        raise CleaningCliError(f"--interim-csv debe estar dentro de data/interim: {interim_csv}")
    _reject_symlinked_interim_source(interim_root, relative_source, interim_csv)
    return source
...
def _reject_symlinked_interim_source(interim_root: Path, relative_source: Path, original_source: Path) -> None:
    current = interim_root
    if current.is_symlink():
        raise CleaningCliError(f"--interim-csv debe estar dentro de data/interim: {original_source}")
