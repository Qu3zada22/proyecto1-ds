...
| `TELEFONO` | 946 faltantes (7.97%), 201 teléfonos no vacíos con caracteres distintos de dígitos y 8 textos sospechosos. | Mantener como texto; separar limpieza básica de espacios de una revisión manual de caracteres no numéricos, extensiones o varios teléfonos en un mismo campo. | El teléfono no debe convertirse a número por ceros, extensiones o separadores; los caracteres pueden indicar información útil. | Alto: eliminar caracteres sin revisión puede perder extensiones o múltiples contactos. | `manual` | `diagnostico_columnas.csv` fila `TELEFONO`; `problemas_potenciales.csv` filas `faltantes_detectados`, `formato_sospechoso` y `texto_sospechoso`; `dominios_observados.csv` valores frecuentes. |
